...
GROUP_RATE=$(ls auto-tests/*-ok.mml | sed -e 's=auto-tests/==g' | cut -d- -f1 | sort -u | wc -l)
LANG=mml
...
function test_prog() {
  ...
  # try to link
  echo -n "LD: "
  ld -melf_i386 -o $TEST_NAME $TEST_NAME.o -L. -L$ROOT/usr/lib -lrts
  if [ $? -ne 0 ]; then
    echo "'ld' failed"
    return
  fi
...
echo -n "[20D[0K"$1" starting" > /dev/fd/6
...
echo -n "[20D[0K"$1" compiling" > /dev/fd/6
compile_project $1
...
for i in $TESTS_DIR/*.${LANG}; do
  echo -n "[50D[0K"$1" "testing $i:t:r" " >> /dev/fd/6
  test_prog $1 $i
done
echo "" >> $RESULTS_FILE
cd $BASE_DIR # just to be more generic
...
echo "[50D[0K"$1" "$total_points" %" > /dev/fd/6
